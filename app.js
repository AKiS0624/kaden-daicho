  ...
  return items
    .filter((it) => {
      ...
      if (query.trim()) {
        ...
        const hay = [padNumber(it.number), it.name, it.modelNumber, it.category, it.location, it.purchasePlace, it.memo]
          .join(" ").toLowerCase();
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            ...
            <div class="font-semibold text-[15px] truncate">
              ${escapeHtml(it.name)}
              ${it.modelNumber ? `<span class="ml-1.5 font-mono-plate text-[11px] font-normal text-[#6B7280]">${escapeHtml(it.modelNumber)}</span>` : ""}
            </div>
...
function openAddForm() {
  ["f_name", "f_modelNumber", "f_purchaseDate", "f_purchasePlace", "f_price", "f_location", "f_warrantyMonths", "f_memo"]
...
function openDetail(id) {
  ...
  document.getElementById("d_rows").innerHTML = rows.map(([label, value]) => `
